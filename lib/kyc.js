...
class kyc extends Contract {
  ...
  async testingDB(ctx){
    con.connect(async function (err, client) {
      if (err) {
          console.error('error connecting: ' + err);
          return;
      }
      let myquery1 = "INSERT INTO cusData (pan ,cname, cmail, cphone, hash, kraOwner) VALUES ('test','test','test','test','test','test')";
      var newRecords = [];
